	...
	<view class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
		...
		<view class="fixed-bottom safe-area-inset-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
			...
			<block v-else-if="!!imgSrc">
				...
				<view class="rechoose" @click="close">取消</view>
				<button class="button" size="mini" @click="cropClick">确定</button>
			</block>
			<view v-else style="display: flex;width: 100%;">
				<view class="choose-btn" @click="close">取消</view>
...
.image-cropper .img-canvas {
    position: absolute !important;
    transform: translateX(-100%);
}
.image-cropper .pic-preview {
    width: 100%;
    flex: 1;
    position: relative;
}
.image-cropper .pic-preview .crop-mask-block {
...
.image-cropper .pic-preview .crop-circle-box {
    position: fixed;
    box-sizing: border-box;
    z-index: 2;
    pointer-events: none;
...
.image-cropper .pic-preview .crop-circle-box .crop-circle {
    width: 100%;
    height: 100%;
}
.image-cropper .pic-preview .crop-image {
    padding: 0 !important;
    margin: 0 !important;
...
.image-cropper .pic-preview .crop-border {
    position: fixed;
...
.image-cropper .pic-preview .crop-grid {
    position: fixed;
    z-index: 3;
    border-style: dashed;
    border-color: #fff;
...
.image-cropper .pic-preview .crop-angle {
    position: fixed;
    z-index: 3;
    border-style: solid;
    border-color: #fff;
...
.image-cropper .fixed-bottom {
    ...
    display: flex;
    flex-direction: row;
    background-color: #f5f5f5; /* 替换 $uni-bg-color-grey */
}
.image-cropper .fixed-bottom .action-bar {
    position: absolute;
    top: -90rpx;
    left: 10rpx;
    display: flex;
}
.image-cropper .fixed-bottom .action-bar .rotate-icon {
...
.image-cropper .fixed-bottom .action-bar .rotate-icon.is-reverse {
    transform: rotateY(180deg);
}
.image-cropper .fixed-bottom .rechoose {
    color: #007aff; /* 替换 $uni-color-primary */
    padding: 0 30rpx; /* 替换 $uni-spacing-row-lg */
    line-height: 100rpx;
}
.image-cropper .fixed-bottom .choose-btn {
    color: #007aff; /* 替换 $uni-color-primary */
    text-align: center;
    line-height: 100rpx;
    flex: 1;
}
.image-cropper .fixed-bottom .button {
    margin: auto 30rpx auto auto; /* 替换 $uni-spacing-row-lg */
    background-color: #007aff; /* 替换 $uni-color-primary */
    color: #fff;
}
.image-cropper .safe-area-inset-bottom {
    padding-bottom: 0;
